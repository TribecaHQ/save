...
generate_declaration_file() {
    ...
    cat artifacts/idl/$prog.ts | sed -E "s/${PREFIX} =/${PREFIX}IDLType =/" | sed -E "s/const IDL: /const ${PREFIX}IDL: /" >>$OUT_PATH

    # error type
    echo "import { generateErrorMap } from '@saberhq/anchor-contrib';" >>$OUT_PATH
    echo "export const ${PREFIX}Errors = generateErrorMap(${PREFIX}IDL);" >>$OUT_PATH
}
